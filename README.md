...
12. **Audit and preview ROM library organization**: Click **Audit ROM Layout...** to compare recorded modern ROM assets with the configured output directory and type/difficulty folder layout
   ...
   - Legacy `file_path` records with recorded identity-migration history can use **Review Provenance...** to explicitly choose whether the ROM belongs to the current SMWC ID or one of the numeric prior IDs already recorded in Collection history. This decision step is read-only and does not call KaizOFF, hash the ROM, or write `files[]` metadata
