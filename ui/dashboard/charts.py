...
from ui.dashboard.time_chart import TimeProgressionChart

class DashboardCharts:
    def __init__(self, parent, analytics_data, view_state=None):
        self.parent = parent
        self.analytics_data = analytics_data
        self.view_state = view_state if view_state is not None else TimeChartViewState()
    ...
    def _create_time_progression_chart(self, parent):
        """Create the interactive chart with session-persistent display choices."""
        self.time_chart = TimeProgressionChart(self.analytics_data, self.view_state, get_colors())
        return self.time_chart.create(parent)

    def _draw_time_progression_lines(self, canvas, filter_type='All Types'):
        """Compatibility entry point for direct chart rendering."""
        TimeProgressionChart(self.analytics_data, self.view_state, get_colors()).draw(canvas, filter_type)
