...
class NavigationBar:
    ...
    def show_page(self, page_name):
        ...
        # Refresh collection when Collection tab is clicked
            ...
            self.root.after(50, self._refresh_planner_if_available)
    ...
    def _refresh_planner_if_available(self):
        """Refresh Planner data if the Planner page is available."""
        try:
            if (
                hasattr(self.root, "main_layout")
                and hasattr(self.root.main_layout, "planner_page")
            ):
                planner_page = self.root.main_layout.planner_page
                if hasattr(planner_page, "refresh") and callable(
                    planner_page.refresh
                ):
                    planner_page.refresh()
        except Exception:
            # Navigation must remain usable even when a page refresh fails.
            pass
