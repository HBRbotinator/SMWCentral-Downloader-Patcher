...
12. **Audit and preview ROM library organization**: Click **Audit ROM Layout...** to compare recorded modern ROM assets with the configured output directory and type/difficulty folder layout
   ...
   - Legacy `file_path` records with recorded identity-migration history can use **Review Provenance...** to explicitly choose whether the ROM belongs to the current SMWC ID or one of the numeric prior IDs already recorded in Collection history. After saving those detached choices, **Preview Modernization Plan...** revalidates exact Collection/path ownership, hashes the ROM under stable filesystem-stat checks, and freezes a proposed primary `files[]` row with the selected SMWC provenance. This remains read-only; Collection metadata is not written yet
