...
8. **Wait for completion**: The app will automatically download each hack and apply it to your base ROM
   ...
   - SMWC community ratings are stored in the canonical Collection `rating` field. v5.1 also repairs the short-lived accidental `smwc_rating` storage without overwriting an already-valid canonical rating.
...
12. **Audit and preview ROM library organization**: Click **Audit ROM Layout...** to compare recorded modern ROM assets with the configured output directory and type/difficulty folder layout
   ...
   - Retained ROMs whose per-file SMWC provenance belongs to a different submission never inherit the current record's catalogue layout; they enter the explicit historical-provenance workflow instead
   ...
   - After explicit save dispositions, **Preview Final Execution Plan...** re-discovers the exact save evidence, revalidates ROM bytes/targets, hashes migrated saves, and freezes the final historical ROM/save execution plan; **Apply Historical Organization...** then uses the same journaled copy → verify → Collection commit → source cleanup transaction as ordinary organization while rechecking the ROM asset's own historical SMWC provenance
   - Historical move plans can then **Review Save Dispositions...** through the same conservative save-impact workflow as ordinary plans. The review uses the already-frozen historical targets and binds decisions to the exact historical SMWC ID/layout metadata plus ROM byte preconditions; final execution planning and transactional filesystem Apply remain separate explicit steps
   - Numeric modern ROMs with missing per-file provenance can use **Review Missing Provenance and, after saving explicit decisions, Apply Provenance Repair...** to explicitly choose the owning SMWC submission for each ROM asset. Choices are limited to the current numeric Collection ID and numeric prior/migration IDs already recorded in Collection history; the review is detached and read-only, with no provider lookup, hashing, metadata write, or move plan yet
   - Legacy `file_path` records with recorded identity-migration history can use **Review Provenance...** to explicitly choose whether the ROM belongs to the current SMWC ID or one of the numeric prior IDs already recorded in Collection history. After saving those detached choices, **Preview Modernization Plan...** revalidates exact Collection/path ownership, hashes the ROM under stable filesystem-stat checks, and freezes a proposed primary `files[]` row with the selected SMWC provenance. **Apply Reviewed Metadata Backfill...** then revalidates the recorded provenance choice and exact ROM bytes again before atomically writing only that frozen `files[]` metadata; ROM/save files, `file_path`, and `additional_paths` remain unchanged
   - Unknown/non-numeric legacy identities and migrated records without a recorded alternative SMWC ID remain blocked rather than receiving a guessed provenance choice
...
Direct SMWC search is reserved for explicit manual fallback when KaizOFF cannot
satisfy the request. Save files are never modified, and no collection change is
made until **Apply Selected** is pressed.
